...
bool operator==(const Player &a, const Player &b) {
  return (b.name == a.name and b.price == a.price and b.points == a.points and
...
public:
  ...
  bool operator()(const Player &p1, const Player &p2) const {
    if (p1.points == 0 and not(p2.points == 0))
      return false;
    if (p2.points == 0 and not(p1.points == 0))
      return true;
...
bool is_player_valid(const Team &team, const Restrictions &restrictions,
                     const Player &p, const vector<int> &size_pos) {
  return (p.price + team.T <= restrictions.T and p.price <= restrictions.J and
          size_pos[p.position] < restrictions.limits[p.position] and
          not team.isOnTeam(p));
}

// Randomly select a player from the Candidate List with the position 'pos'
...
Team greedy_randomized_team(Players &all_players,
  ...
  PlayersByPosition players = {
      Players(restrictions.limits[0]), Players(restrictions.limits[1]),
      Players(restrictions.limits[2]), Players(restrictions.limits[3])};
  ...
  for (int total_players = 0; total_players < 11; ++total_players) {
    ...
    // Generate a candidate list with the first alpha available players from
    // 'all_players' (already sorted) We introduced the second condition in the
    // event that there are no alpha players available to add.
...
Team find_random_neighbour(Team team, const Players &all_players,
                           const Restrictions &restrictions) {
...
// Defines the probability of accepting a worsening move using Boltzman
// probability
double boltzman_probability(const int &punts1, const int &punts2,
                            const int &T) {
...
*/
Team local_search(const Team &team, const Players &all_players,
                  const Restrictions &restrictions) {
...
void metaheuristic(const Restrictions &restrictions, Players &all_players,
